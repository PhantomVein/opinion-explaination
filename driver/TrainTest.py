...
def train(train_inst, dev_data, test_data, model, vocab, config):
    ...
    for iter in range(config.train_iters):
        ...
        for one_batch in data_iter(train_inst, config.train_batch_size, True):
            ...
            batch_features, batch_features_length, batch_gold_label = batch_data_variable(one_batch, vocab, config)
...
if __name__ == '__main__':
    ...
    biLSTM = BiLSTM(embedding, config).to(config.device)

    # parser = DisParser(pwordEnc, wordLSTM, sent2span, EDULSTM, dec, config)
